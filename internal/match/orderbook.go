...
import (
	...
	"lightning-engine/mq"
	depthOrderbook "lightning-engine/orderbook"
...
type Orderbook struct {
	...
	DepthOrderbook *depthOrderbook.OrderBook
}
...
func NewOrderbook(status *status.Status, pair string, mq mq.IMQ) (*Orderbook, error) {
	...
	depthOrderBook := depthOrderbook.NewOrderBook(mq, pair)
	return &Orderbook{
		pair:     pair,
		bid:      bid,
		ask:      ask,
...
func (ob *Orderbook) addBidLimitGTC(order models.Order) error {
	ob.DepthOrderbook.ProcessLimitOrderUsingOrder(order)
...
// cancel 撤单
func (ob *Orderbook) cancel(id string) error {
	ob.DepthOrderbook.CancelOrderOutside(id)
