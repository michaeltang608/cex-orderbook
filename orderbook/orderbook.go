...
func NewOrderBook(mq mq.IMQ, symbol string) *OrderBook {
	orderBook := &OrderBook{
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				orderBook.PushDepthLevelOverview()
			}
		}
	}()
	return orderBook
...
// PushDepthLevelOverview 每隔100毫秒推送深度全量行情
func (ob *OrderBook) PushDepthLevelOverview() {
	ob.mutex.Lock()
	marketOverview := ob.MarketOverview()
	ob.mutex.Unlock()
	ob.mq.PushMarketOverview(marketOverview)
}

// ProcessLimitOrderUsingOrder 用于处理每笔订单
func (ob *OrderBook) ProcessLimitOrderUsingOrder(order models.Order) {
	if order.Side == models.Buy {
		ob.mutex.Lock()
		_, _, _, err := ob.ProcessLimitOrder(Buy, order.Id, order.Amount, order.Price)
		ob.mutex.Unlock()
		if err != nil {
			log.Info("[ProcessLimitOrderUsingOrder] buy order %+v error:%+v\n", order, err)
		}
	}
	if order.Side == models.Sell {
		ob.mutex.Lock()
		_, _, _, err := ob.ProcessLimitOrder(Sell, order.Id, order.Amount, order.Price)
		ob.mutex.Unlock()
		if err != nil {
			log.Info("[ProcessLimitOrderUsingOrder] sell order %+v error:%+v\n", order, err)
		}
	}
...
func (ob *OrderBook) CancelOrderOutside(orderID string) {
	ob.mutex.Lock()
